...
def convresponse_n0(image: np.ndarray, shapelet_order: Union[str,int] = 'default', verbose: bool = True):
    r""" 
    This function computes the convolution between a range of shapelets kernels and an image, extracting the magnitude of response as well as the shapelet-based orientation as per the steerable shapelet formulism[RS]_.
    ...
    * shapelet_order: Union[str,int]
        * Set as 'default' to use higher-order shapelets[MT]_ ($m \leq m'$). Can also accept integer value such that analysis uses $m \in [1, shapelet_{order}]$
    ...
    * phi: numpy.ndarray
    ...
    Notes
    -----
    This function uses the orthonormal polar shapelet definition for $n=0$ shapelets[TA]_ (see shapelets.functions.orthonormalpolar2D_n0).

    References
    ----------
    .. [RS] http://dx.doi.org/10.1103/PhysRevE.91.033307
    .. [MT] http://dx.doi.org/10.1088/1361-6528/ad1df4
    .. [TA] https://doi.org/10.1088/1361-6528/aaf353
...
def convresponse_n1(image: np.ndarray, mmax: int, verbose=True):
    ...
    * phi: numpy.ndarray
    ...
    Notes
    -----
    This function uses the orthonormal polar shapelet definition for $n=1$ shapelets[MT]_ (see shapelets.functions.orthonormalpolar2D_n1).

    References
    ----------
    .. [RS] http://dx.doi.org/10.1103/PhysRevE.91.033307
    .. [MT] https://hdl.handle.net/10012/20779
